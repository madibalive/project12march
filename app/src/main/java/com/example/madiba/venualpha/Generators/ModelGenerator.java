package com.example.madiba.venualpha.Generators;
...
public class ModelGenerator {
    ...
    public static MdUserItem generateUser(ParseUser user){
        ...
        if (user.getParseFile("avatar50")!=null)
            item.setAvatarSmall(user.getParseFile("avatar50").getUrl());
        if (user.getParseFile("avatar100")!=null)
            item.setAvatarLarge(user.getParseFile("avatar100").getUrl());

        item.setDateToString(TimeUitls.getRelativeTime(user.getCreatedAt()));
        return item;
    }

    public static MdEventItem generateSimpleEvent(ParseObject object){
        MdEventItem item = new MdEventItem();

        item.setParseId(object.getObjectId());
        item.setHashtag(object.getString("hashTag"));
