...
public class DiscoverAdapter extends BaseQuickAdapter<DiscoverModel> {
    ...
    protected BaseViewHolder onCreateDefViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == DiscoverModel.CATEGORIES)
            return new CategoriesViewHolder(getItemView(R.layout.container_box,parent));
        else if (viewType == DiscoverModel.TOP_PEOPLE)
            return new TopPeopleViewHolder(getItemView(R.layout.container_box_header, parent));
        else if (viewType == DiscoverModel.New_PEOPLE)
            return new TopPeopleViewHolder(getItemView(R.layout.container_box_header, parent));
        else if (viewType == DiscoverModel.DIS_EXPLORE)
            return new DiscoverExploreViewHolder(getItemView(R.layout.container_box_header, parent));
        else if (viewType == DiscoverModel.NEW_EVENT)
            return new NewEventViewHolder(getItemView(R.layout.container_box_header, parent));
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position, List<Object> payloads) {
        ...
        if (holder instanceof CategoriesViewHolder) {
            ...
            mAdapter.setOnRecyclerViewItemClickListener((view, i) -> {
                NavigateTo.goToCategory(mContext,mAdapter.getItem(i).getmName(),i+1);
            });
...
//            });
            ...
            TextView rl = ((DiscoverExploreViewHolder) holder).getView(R.id.box_title);
            rl.setText("New Peeps");
            RecyclerView rview = ((DiscoverExploreViewHolder) holder).getView(R.id.box_recyclerview);
            ...
            TextView rl = ((NewEventViewHolder) holder).getView(R.id.box_title);
            rl.setText("New Peeps");
            RecyclerView rview = ((NewEventViewHolder) holder).getView(R.id.box_recyclerview);
            SuggestedBasedOnFriendEvens mAdapter = new SuggestedBasedOnFriendEvens(R.layout.invite_event, n.getListObject());
            rview.setLayoutManager(new LinearLayoutManager(mContext, LinearLayoutManager.HORIZONTAL, false));
            rview.setAdapter(mAdapter);
            mAdapter.setOnRecyclerViewItemClickListener((view, i) -> {
                SingletonDataSource.getInstance().setCurrentEvent(mAdapter.getItem(i));
                NavigateTo.goToEventPage(mContext,mAdapter.getItem(i).getObjectId(),mAdapter.getItem(i).getClassName(),n.getListObject().get(i).getInt("theme"));
            });
